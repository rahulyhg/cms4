...
class PclZip
{
    ...
    public function privParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options = false)
    {
        ...
        while ($index < $p_size) {
            ...
            if (!isset($v_requested_options[$p_options_list[$index]])) {
                // ----- Error log
                PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, 'Invalid optional parameter \'' . $p_options_list[$index] . '\' for this method');
            ...
            switch ($p_options_list[$index]) {
                ...
                case PCLZIP_OPT_ADD_PATH:
                    ...
                    if (($index + 1) >= $p_size) {
                        // ----- Error log
                        PclZip::privErrorLog(PCLZIP_ERR_MISSING_OPTION_VALUE, 'Missing parameter value for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '"');
                    ...
                    // ----- Get the value
                    $v_result_list[$p_options_list[$index]] = PclZipUtilTranslateWinPath($p_options_list[$index + 1], false);
                    $index++;
                    break;
                ...
                case PCLZIP_OPT_TEMP_FILE_THRESHOLD:
                    ...
                    if (isset($v_result_list[PCLZIP_OPT_TEMP_FILE_OFF])) {
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, 'Option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\' can not be used with option \'PCLZIP_OPT_TEMP_FILE_OFF\'');

                        return PclZip::errorCode();
                    }

                    // ----- Check the value
                    $v_value = $p_options_list[$index + 1];
                    if ((!is_integer($v_value)) || ($v_value < 0)) {
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Integer expected for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');

                        return PclZip::errorCode();
                    }

                    // ----- Get the value (and convert it in bytes)
                    $v_result_list[$p_options_list[$index]] = $v_value * 1048576;
                    $index++;
                    break;
                ...
                case PCLZIP_OPT_TEMP_FILE_ON:
                    ...
                    $v_result_list[$p_options_list[$index]] = true;
                    break;

                case PCLZIP_OPT_TEMP_FILE_OFF:
                    // ----- Check for incompatible options
                    if (isset($v_result_list[PCLZIP_OPT_TEMP_FILE_ON])) {
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, 'Option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\' can not be used with option \'PCLZIP_OPT_TEMP_FILE_ON\'');

                        return PclZip::errorCode();
                    }
                    // ----- Check for incompatible options
                    if (isset($v_result_list[PCLZIP_OPT_TEMP_FILE_THRESHOLD])) {
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, 'Option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\' can not be used with option \'PCLZIP_OPT_TEMP_FILE_THRESHOLD\'');
                ...
                case PCLZIP_OPT_EXTRACT_DIR_RESTRICTION:
                    ...
                    // ----- Get the value
                    if (is_string($p_options_list[$index + 1]) && ($p_options_list[$index + 1] != '')) {
                        $v_result_list[$p_options_list[$index]] = PclZipUtilTranslateWinPath($p_options_list[$index + 1], false);
                        $index++;
                    }
                    break;
                ...
                case PCLZIP_OPT_BY_NAME:
                    ...
                    // ----- Get the value
                    if (is_string($p_options_list[$index + 1])) {
                        $v_result_list[$p_options_list[$index]][0] = $p_options_list[$index + 1];
                    } elseif (is_array($p_options_list[$index + 1])) {
                        $v_result_list[$p_options_list[$index]] = $p_options_list[$index + 1];
                    } else {
                        // ----- Error log
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Wrong parameter value for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');
                ...
                case PCLZIP_OPT_BY_INDEX:
                    ...
                    if (is_string($p_options_list[$index + 1])) {
                        ...
                        // ----- Remove spaces
                        $p_options_list[$index + 1] = strtr($p_options_list[$index + 1], ' ', '');

                        // ----- Parse items
                        $v_work_list = explode(',', $p_options_list[$index + 1]);
                    } elseif (is_integer($p_options_list[$index + 1])) {
                        $v_work_list[0] = $p_options_list[$index + 1] . '-' . $p_options_list[$index + 1];
                    } elseif (is_array($p_options_list[$index + 1])) {
                        $v_work_list = $p_options_list[$index + 1];
                    } else {
                        // ----- Error log
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Value must be integer, string or array for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');
                    ...
                    $v_sort_flag = false;
                    $v_sort_value = 0;
                    for ($jndex = 0; $jndex < count($v_work_list); $jndex++) {
                        // ----- Explode the item
                        $v_item_list = explode('-', $v_work_list[$jndex]);
                        ...
                        if ($v_size_item_list == 1) {
                            ...
                            $v_result_list[$p_options_list[$index]][$jndex]['start'] = $v_item_list[0];
                            $v_result_list[$p_options_list[$index]][$jndex]['end'] = $v_item_list[0];
                        } elseif ($v_size_item_list == 2) {
                            // ----- Set the option value
                            $v_result_list[$p_options_list[$index]][$jndex]['start'] = $v_item_list[0];
                            $v_result_list[$p_options_list[$index]][$jndex]['end'] = $v_item_list[1];
                        } else {
                            // ----- Error log
                            PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Too many values in index range for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');
                        ...
                        // ----- Look for list sort
                        if ($v_result_list[$p_options_list[$index]][$jndex]['start'] < $v_sort_value) {
                            $v_sort_flag = true;

                            // ----- TBC : An automatic sort should be writen ...
                            // ----- Error log
                            PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Invalid order of index range for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');

                            // ----- Return
                            return PclZip::errorCode();
                        }
                        $v_sort_value = $v_result_list[$p_options_list[$index]][$jndex]['start'];
                    }
                ...
                case PCLZIP_CB_POST_ADD:
                    ...
                    */
                    ...
                    // ----- Get the value
                    $v_function_name = $p_options_list[$index + 1];

                    // ----- Check that the value is a valid existing function
                    if (!function_exists($v_function_name)) {
                        // ----- Error log
                        PclZip::privErrorLog(PCLZIP_ERR_INVALID_OPTION_VALUE, 'Function \'' . $v_function_name . '()\' is not an existing function for option \'' . PclZipUtilOptionText($p_options_list[$index]) . '\'');
                    ...
                    // ----- Set the attribute
                    $v_result_list[$p_options_list[$index]] = $v_function_name;
                    $index++;
                    break;

                default:
                    // ----- Error log
                    PclZip::privErrorLog(PCLZIP_ERR_INVALID_PARAMETER, 'Unknown parameter \'' . $p_options_list[$index] . '"');
    ...
    public function privFileDescrExpand(&$p_filedescr_list, &$p_options)
    {
        ...
        // ----- Look each entry
        for ($index = 0; $index < count($p_filedescr_list); $index++) {

            // ----- Get filedescr
            $v_descr = $p_filedescr_list[$index];
    ...
    public function privAddList($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        for ($index = 0, $v_count = 0; $index < count($v_header_list); $index++) {
            // ----- Create the file header
            if ($v_header_list[$index]['status'] == 'ok') {
                if (($v_result = $this->privWriteCentralFileHeader($v_header_list[$index])) != 1) {
                    // ----- Return
                    return $v_result;
                }
                $v_count++;
            ...
            $this->privConvertHeader2FileInfo($v_header_list[$index], $p_result_list[$index]);
        }
    ...
    public function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
    {
        ...
        // ----- Loop on the files
        for ($jndex = 0; ($jndex < count($p_filedescr_list)) && ($v_result == 1); $jndex++) {
            // ----- Format the filename
            $p_filedescr_list[$jndex]['filename'] = PclZipUtilTranslateWinPath($p_filedescr_list[$jndex]['filename'], false);

            // ----- Skip empty file names
            // TBC : Can this be possible ? not checked in DescrParseAtt ?
            if ($p_filedescr_list[$jndex]['filename'] == '') {
                continue;
            }

            // ----- Check the filename
            if (($p_filedescr_list[$jndex]['type'] != 'virtual_file') && (!file_exists($p_filedescr_list[$jndex]['filename']))) {
                PclZip::privErrorLog(PCLZIP_ERR_MISSING_FILE, 'File \'' . $p_filedescr_list[$jndex]['filename'] . '\' does not exist');
            ...
            //      if (   (is_file($p_filedescr_list[$jndex]['filename']))
            //          || (   is_dir($p_filedescr_list[$jndex]['filename'])
            if (($p_filedescr_list[$jndex]['type'] == 'file') || ($p_filedescr_list[$jndex]['type'] == 'virtual_file') || (($p_filedescr_list[$jndex]['type'] == 'folder') && (!isset($p_options[PCLZIP_OPT_REMOVE_ALL_PATH]) || !$p_options[PCLZIP_OPT_REMOVE_ALL_PATH]))) {

                // ----- Add the file
                $v_result = $this->privAddFile($p_filedescr_list[$jndex], $v_header, $p_options);
                if ($v_result != 1) {
                    return $v_result;
                }
    ...
    public function privList(&$p_list)
    {
        ...
        // ----- Read each entry
        for ($index = 0; $index < $v_central_dir['entries']; $index++) {
            ...
            $v_header['index'] = $index;

            // ----- Get the only interesting attributes
            $this->privConvertHeader2FileInfo($v_header, $p_list[$index]);
            unset($v_header);
        }
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        // ----- Read each entry
        $jndex_start = 0;
        for ($index = 0, $v_nb_extracted = 0; $index < $v_central_dir['entries']; $index++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($jndex = 0; ($jndex < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $jndex++) {

                    // ----- Look for a directory
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$jndex], -1) == '/') {

                        // ----- Look if the directory is in the filename path
                        if ((strlen($v_header['stored_filename']) > strlen($p_options[PCLZIP_OPT_BY_NAME][$jndex])) && (substr($v_header['stored_filename'], 0, strlen($p_options[PCLZIP_OPT_BY_NAME][$jndex])) == $p_options[PCLZIP_OPT_BY_NAME][$jndex])) {
                            $v_extract = true;
                        }

                        // ----- Look for a filename
                    } elseif ($v_header['stored_filename'] == $p_options[PCLZIP_OPT_BY_NAME][$jndex]) {
                        $v_extract = true;
                    }
                }
                ...
                */
                ...
                for ($jndex = $jndex_start; ($jndex < count($p_options[PCLZIP_OPT_BY_INDEX])) && (!$v_extract); $jndex++) {
                    if (($index >= $p_options[PCLZIP_OPT_BY_INDEX][$jndex]['start']) && ($index <= $p_options[PCLZIP_OPT_BY_INDEX][$jndex]['end'])) {
                        $v_extract = true;
                    }
                    if ($index >= $p_options[PCLZIP_OPT_BY_INDEX][$jndex]['end']) {
                        $jndex_start = $jndex + 1;
                    }

                    if ($p_options[PCLZIP_OPT_BY_INDEX][$jndex]['start'] > $index) {
                        break;
                    }
                }
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (!(($p_entry['external'] & 0x00000010) == 0x00000010)) {
                ...
                if ($p_entry['compression'] == 0) {
                    ...
                    // ----- Reading the file
                    @fread($this->zip_fd, $p_entry['compressed_size']);
                }
    ...
    public function privExtractFileInOutput(&$p_entry, &$p_options)
    {
        ...
        // ----- Read the file header
        if (($v_result = $this->privReadFileHeader()) != 1) {
            return $v_result;
        }
    ...
    public function privDeleteByRule(&$p_result_list, &$p_options)
    {
        ...
        for ($index = 0, $v_nb_extracted = 0; $index < $v_central_dir['entries']; $index++) {
            ...
            // ----- Store the index
            $v_header_list[$v_nb_extracted]['index'] = $index;
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                // ----- Look if the filename is in the list
                for ($jndex = 0; ($jndex < count($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $jndex++) {

                    // ----- Look for a directory
                    if (substr($p_options[PCLZIP_OPT_BY_NAME][$jndex], -1) == '/') {

                        // ----- Look if the directory is in the filename path
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen($p_options[PCLZIP_OPT_BY_NAME][$jndex])) && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, strlen($p_options[PCLZIP_OPT_BY_NAME][$jndex])) == $p_options[PCLZIP_OPT_BY_NAME][$jndex])) {
                            $v_found = true;
                        } elseif ((($v_header_list[$v_nb_extracted]['external'] & 0x00000010) == 0x00000010) /* Indicates a folder */ && ($v_header_list[$v_nb_extracted]['stored_filename'] . '/' == $p_options[PCLZIP_OPT_BY_NAME][$jndex])) {
                            $v_found = true;
                        }

                        // ----- Look for a filename
                    } elseif ($v_header_list[$v_nb_extracted]['stored_filename'] == $p_options[PCLZIP_OPT_BY_NAME][$jndex]) {
                        $v_found = true;
                    }
                }
                ...
                */
                ...
                // ----- Look if the index is in the list
                for ($jndexndex = $jndex_start; ($jndexndex < count($p_options[PCLZIP_OPT_BY_INDEX])) && (!$v_found); $jndexndex++) {
                    if (($index >= $p_options[PCLZIP_OPT_BY_INDEX][$jndexndex]['start']) && ($index <= $p_options[PCLZIP_OPT_BY_INDEX][$jndexndex]['end'])) {
                        $v_found = true;
                    }

                    if ($p_options[PCLZIP_OPT_BY_INDEX][$jndexndex]['start'] > $index) {
                        break;
                    }
                }
            } else {
        ...
        if ($v_nb_extracted > 0) {
            ...
            for ($index = 0; $index < count($v_header_list); $index++) {

                // ----- Calculate the position of the header
                @rewind($this->zip_fd);
                if (@fseek($this->zip_fd, $v_header_list[$index]['offset'])) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                ...
                if (($v_result = $v_temp_zip->privWriteFileHeader($v_header_list[$index])) != 1) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                    $v_temp_zip->privCloseFd();
                    @unlink($v_zip_temp_name);
                ...
                if (($v_result = PclZipUtilCopyBlock($this->zip_fd, $v_temp_zip->zip_fd, $v_header_list[$index]['compressed_size'])) != 1) {
                    // ----- Close the zip file
                    $this->privCloseFd();
                    $v_temp_zip->privCloseFd();
                    @unlink($v_zip_temp_name);
            ...
            for ($index = 0; $index < count($v_header_list); $index++) {
                // ----- Create the file header
                if (($v_result = $v_temp_zip->privWriteCentralFileHeader($v_header_list[$index])) != 1) {
                    $v_temp_zip->privCloseFd();
                    $this->privCloseFd();
                    @unlink($v_zip_temp_name);
                ...
                // ----- Transform the header to a 'usable' info
                $v_temp_zip->privConvertHeader2FileInfo($v_header_list[$index], $p_result_list[$index]);
            }
    ...
    public function properties()
    {
        ...
        if (@is_file($this->zipname)) {
            ...
            // ----- Read the central directory informations
            $v_central_dir = [];
            if ($this->privReadEndCentralDir($v_central_dir) != 1) {
                $this->privSwapBackMagicQuotes();
...
function PclZipUtilPathReduction($p_dir)
{
    ...
    if ($p_dir != '') {
        ...
        $v_skip = 0;
        for ($index = count($v_list) - 1; $index >= 0; $index--) {
            ...
            if ($v_list[$index] == '..') {
                ...
                } elseif ($index == (count($v_list) - 1)) {
                    $v_result = $v_list[$index];
                ...
                if ($v_skip > 0) {
                    ...
                    $v_result = $v_list[$index] . ($index != (count($v_list) - 1) ? '/' . $v_result : '');
                }
            }
        }
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    $index = 0;
    $jndexndex = 0;
    while (($index < $v_list_dir_size) && ($jndexndex < $v_list_path_size) && ($v_result)) {
        ...
        if ($v_list_path[$jndexndex] == '') {
            $jndexndex++;
            continue;
        }

        // ----- Compare the items
        if (($v_list_dir[$index] != $v_list_path[$jndexndex]) && ($v_list_dir[$index] != '') && ($v_list_path[$jndexndex] != '')) {
            $v_result = 0;
        }
    ...
    if ($v_result) {
        ...
        while (($jndexndex < $v_list_path_size) && ($v_list_path[$jndexndex] == '')) {
            $jndexndex++;
        }
        while (($index < $v_list_dir_size) && ($v_list_dir[$index] == '')) {
            $index++;
        }

        if (($index >= $v_list_dir_size) && ($jndexndex >= $v_list_path_size)) {
            // ----- There are exactly the same
            $v_result = 2;
        } elseif ($index < $v_list_dir_size) {
