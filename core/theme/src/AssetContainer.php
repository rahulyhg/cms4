...
class AssetContainer
{
    ...
    protected function configAssetUrl($path, $secure = null)
    {
        ...
        if (empty($assetUrl)) {
            $assetUrl = Config::get('core.theme.general.assetUrl', '');
        }
        ...
        if ($assetUrl) {
            ...
            $basePath = str_contains($base, $index) ? str_replace('/' . $index, '', $base) : $base;
        } else {
            if (empty($secure)) {
            ...
            // Asset URL without index.
            $basePath = str_contains($root, $index) ? str_replace('/' . $index, '', $root) : $root;
        }
