        ...
        <ul>
            @foreach($children['root'] as $element_key => $element)
                <li class="collapsed" id="node{{ $element_key }}">
                    <input type="checkbox" id="checkSelect{{ $element_key }}" name="flags[]" value="{{ $flags[$element]['flag'] }}" @if (in_array($flags[$element]['flag'], $active)) checked @endif>
                    ...
                    @if (isset($children[$element]))
                        <ul>
                            @foreach($children[$element] as $sub_key => $subElements)
                                <li class="collapsed" id="node_sub_{{ $sub_key }}">
                                    <input type="checkbox" id="checkSelect_sub{{ $sub_key }}" name="flags[]" value="{{ $flags[$subElements]['flag'] }}" @if (in_array($flags[$subElements]['flag'], $active)) checked @endif>
                                    <label for="checkSelect_sub{{ $sub_key }}" class="label label-primary nameMargin">{{ $flags[$subElements]['name'] }}</label>
                                        ...
                                        <ul>
                                            @foreach($children[$subElements] as $sub_sub_key => $subSubElements)
                                                <li class="collapsed" id="node_sub_sub_{{ $sub_sub_key }}">
                                                    <input type="checkbox" id="checkSelect_sub_sub{{ $sub_sub_key }}" name="flags[]" value="{{ $flags[$subSubElements]['flag'] }}" @if (in_array($flags[$subSubElements]['flag'], $active)) checked @endif>
                                                        ...
                                                        <ul>
                                                            @foreach($children[$subSubElements] as $grand_children_key => $grandChildrenElements)
                                                                <li class="collapsed" id="node_grand_child{{ $grand_children_key }}">
                                                                    <input type="checkbox" id="checkSelect_grand_child{{ $grand_children_key }}" name="flags[]" value="{{ $flags[$grandChildrenElements]['flag'] }}" @if (in_array($flags[$grandChildrenElements]['flag'], $active)) checked @endif>
                                                                        ...
                                                                        <ul>
                                                                            @foreach($children[$grandChildrenElements] as $grand_children_key_sub => $greatGrandChildrenElements)
                                                                                <li class="collapsed" id="node{{ $grand_children_key }}">
                                                                                    <input type="checkbox" id="checkSelect_grand_child{{ $grand_children_key_sub }}" name="flags[]" value="{{ $flags[$grandChildrenElements]['flag'] }}" @if (in_array($flags[$grandChildrenElements]['flag'], $active)) checked @endif>
