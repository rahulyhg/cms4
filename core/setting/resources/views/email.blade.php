...
@section('content')
        ...
        <div class="flexbox-annotated-section-content">
            <div class="wrapper-content pd-all-20" id="email-config-form">
                <div class="form-group">
                    ...
                    <select name="email_driver" class="form-control next-input" id="email_driver">
                ...
                <div class="setting-mail-mail-gun @if (setting('email_driver', config('mail.driver')) !== 'mailgun') hidden @endif">
                    <div class="form-group">
                        <label class="text-title-field" for="email_email_gun_domain">{{ trans('core.setting::setting.email.mail_gun_domain') }}</label>
                        <input data-counter="60" type="text" class="next-input" name="email_email_gun_domain" id="email_email_gun_domain"
                               value="{{ setting('email_email_gun_domain', config('services.mailgun.domain')) }}" placeholder="{{ trans('core.setting::setting.email.mail_gun_domain_placeholder') }}">
                    </div>
                    <div class="form-group">
                        <label class="text-title-field" for="email_email_gun_secret">{{ trans('core.setting::setting.email.mail_gun_secret')  }}</label>
                        <input data-counter="60" type="text" class="next-input" name="email_email_gun_secret" id="email_email_gun_secret"
                               value="{{ setting('email_email_gun_secret', config('services.mailgun.secret')) }}" placeholder="{{ trans('core.setting::setting.email.mail_gun_secret_placeholder') }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="text-title-field" for="email_encryption">{{ trans('core.setting::setting.email.encryption') }}</label>
                    <select name="email_encryption" class="form-control next-input" id="email_encryption">
    ...
    <div class="flexbox-annotated-section" style="border: none">
        ...
        <div class="flexbox-annotated-section-content">
            <a class="btn btn-default" data-send-url="{{ route('setting.email.send.test') }}" id="send-test-email-btn" href="#">{{ __('Test sending email') }}</a>
