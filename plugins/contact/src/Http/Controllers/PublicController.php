...
class PublicController extends Controller
{
    ...
    public function postSendContact(ContactRequest $request, BaseHttpResponse $response)
    {
        try {
            ...
            if (setting('plugins_contact_notice_status')) {
                ...
                EmailHandler::send($content, setting('plugins_contact_notice_subject', config('plugins.contact.email.templates.notice.subject')));
            }

            return $response->setMessage(trans('plugins.contact::contact.email.success'));
        } catch (Exception $ex) {
            info($ex->getMessage());
            return $response->setError(true)->setMessage(trans('plugins.contact::contact.email.failed'));
        }
    }
}
